...
class ASC(nn.Module):
    ...
    def __init__(self, a=3.5):
        ...
        self.a=a
    def forward(self, input):
        return torch.div(torch.exp(self.a*input),torch.sum(torch.exp(self.a*input), dim=1)) 
       # return torch.div(torch.exp(x),torch.sum(torch.exp(x), dim=1)) 
       # return torch.div(x,torch.sum(x, dim=1)+1e-20) 
...
def act(act_fun = 'LeakyReLU'):
    ...
    if isinstance(act_fun, str):
        if act_fun == 'LeakyReLU':
            ...
            return nn.Sigmoid()
        elif act_fun == 'Hardtanh':
            return nn.Hardtanh(min_val=-15,max_val=15)
        elif act_fun == 'Tanh':
            return nn.Tanh()
